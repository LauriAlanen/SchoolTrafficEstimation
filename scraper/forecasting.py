...
def plot_occurrences(total_occurrences_df):
    total_occurrences_df['end_date'] = pd.to_datetime(
        total_occurrences_df['end_date'])
    total_occurrences_df = total_occurrences_df.sort_values(by='end_date')
    plt.figure(figsize=(10, 6))
    plt.plot(total_occurrences_df['end_date'],
             total_occurrences_df['count'], marker='o', linestyle='-')
    plt.xlabel('End Date')
    plt.ylabel('Count')
...
def use_prophet(path_to_total_occurrences):
    total_occurrences_df = fileOperations.read_df_from_file(
        path_to_total_occurrences)
    total_occurrences_df = total_occurrences_df.reset_index()
    total_occurrences_df['ds'] = pd.to_datetime(
        total_occurrences_df['end_date'])
