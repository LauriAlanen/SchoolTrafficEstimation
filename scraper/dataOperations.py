...
from scraper import calendarRequest
from scraper import fileOperations
from scraper import websiteOperations
...
def gather_and_filter_calendar_information(driver, sub_class, date_from, date_to):
    ...
    calendar = calendarRequest.get_calendar_cookie(
...
def build_total_traffic_df(calendar_df, total_traffic_df):
    ...
    if total_traffic_df.empty:
        ...
        total_traffic_df = pd.concat([total_traffic_df, current_occurrences_df]).groupby(
...
def get_amount_of_people_in_class(all_classes_df, sub_class):
    try:
        ...
        print(
            f"Warning - Unable to get amount of people with class name : {class_name[0]} and sub class : {sub_class}")
        return None
