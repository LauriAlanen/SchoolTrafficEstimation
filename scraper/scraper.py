...
def main():
    ...
    with alive_bar(total_class_count) as bar:
        for class_info_list, class_name, sub_class in current_class:
            ...
            filtered_calendar = do.gather_and_filter_calendar_information(
                driver, sub_class, date_from, date_to)

            if(filtered_calendar is not None):
                fo.save_df_to_file(filtered_calendar,
                                   f"calendars/{class_name}/{sub_class}.csv")
                calendar_df = fo.read_df_from_file(
                    f"calendars/{class_name}/{sub_class}.csv", silent=False)
                total_traffic_df = do.build_total_traffic_df(
