...
def main():
    ...
    with alive_bar(total_class_count) as bar:
        for class_list, class_name, sub_class in current_class:
            ...
            if(filtered_calendar is not None):
                ...
                forecasting.distribute_to_restaurants(class_list[sub_class]["people"], class_list[sub_class]["restaurants"])
                
            print("")
            bar()
            driver.refresh()
    
    fileOperations.save_df_to_file(total_occurrences_df, f"results/predictions/{date_from}_to_{date_to}.csv")
